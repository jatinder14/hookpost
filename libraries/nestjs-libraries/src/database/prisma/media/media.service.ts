...
import { generationError } from '@hookpost/nestjs-libraries/openai/generation.error';
...
import { organizationId } from '@hookpost/nestjs-libraries/temporal/temporal.search.attribute';
import { makeId } from '@hookpost/nestjs-libraries/services/make.is';
import { MediaProcessorJob } from '@hookpost/nestjs-libraries/upload/media.processor.interface';
...
export class MediaService {
  ...
  async uploadFromUrl(org: string, url: string) {
    ...
    // Name the media after the URL's last path segment rather than the random
    // storage key, so it is recognisable in the library (Hookpost change).
    return this.saveFile(
      org,
      uploaded.originalname,
      uploaded.path,
      new URL(url).pathname.split('/').pop() || undefined
    );
  }
