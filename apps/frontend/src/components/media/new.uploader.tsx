...
import { getUppyUploadPlugin } from '@hookpost/react/helpers/uppy.upload';
...
export function useUppyUploader(props: {
  ...
  return useMemo(() => {
    ...
    // The caller flips its spinner on as soon as files are handed to Uppy, so
    // every way an upload can end has to call onEnd or the Upload button stays
    // disabled behind a spinner forever and no further media can be added.
    // 'error' and 'complete' alone miss the two most common endings: a file
    // rejected by the restrictions/onBeforeFileAdded check (which never reaches
    // upload-start) and a per-file transport failure.
    uppy2.on('restriction-failed', () => {
      uppy2.clear();
      setLocked(false);
      props.onEnd();
      fileOrderIndex = 0;
    });
    uppy2.on('upload-error', () => {
      setLocked(false);
      props.onEnd();
    });
