...
import { readOrFetch } from '@hookpost/helpers/utils/read.or.fetch';
...
import { PostPlug } from '@hookpost/helpers/decorators/post.plug';
...
export class LinkedinProvider extends SocialAbstract implements SocialProvider {
  ...
  // LinkedIn rejects the WHOLE authorization with "Bummer, something went wrong"
  // if the app is not approved for a requested scope, and authenticate() then
  // calls checkScopes(), which throws when a requested scope was not granted.
  // So this must list only what the LinkedIn app is actually approved for.
  //
  // The default is the self-serve set every app gets:
  //   openid + profile      -> "Sign In with LinkedIn using OpenID Connect"
  //   w_member_social       -> "Share on LinkedIn" (posting to the member's feed)
  //
  // Deliberately NOT default: r_basicprofile (only supplies vanityName, which
  // degrades gracefully to undefined) and the rw_organization_admin /
  // w_organization_social / r_organization_social trio, which belong to the
  // Community Management API and require LinkedIn Partner Program approval -
  // they only power company @-mentions here, plus the LinkedIn Page provider.
  //
  // Override with LINKEDIN_SCOPES (space or comma separated) once more products
  // are approved, so widening scopes needs no code change.
  ...
  async generateAuthUrl() {
    ...
    // No prompt=none: it forbids LinkedIn from showing the login/consent UI,
    // so first-time connects (no session or prior grant) hard-fail with
