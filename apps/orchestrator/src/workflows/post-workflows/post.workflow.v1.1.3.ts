...
import { PostResponse } from '@hookpost/nestjs-libraries/integrations/social/social.integrations.interface';
...
import { postId as postIdSearchParam } from '@hookpost/nestjs-libraries/temporal/temporal.search.attribute';
...
export async function postWorkflowV113({
  taskQueue,
  postId,
  organizationId,
  postNow = false,
  ...
  // Mark the row that actually failed. Up to V112 every failure - including a
  // failed first comment - was written onto postsList[0], so a main post that
  // had already published (releaseURL saved by updatePost) was flipped back to
  // ERROR and the calendar reported a live post as failed. Facebook made this
  // routine: the reel publishes, then `POST /{id}/comments` is rejected with
  // "(#200) pages_read_user_content ... need to be approved by App Review".
  // The comment row still has to reach a terminal state, otherwise the
  // missing-posts sweep sees a QUEUE row and republishes it.
  const failRow = async (index: number, err: any) => {
    const target = postsList[index] || postsList[0];
    await changeState(
      target.id,
      'ERROR',
      err,
      index === 0 ? postsList : [target]
    );
  };
  ...
  const markUnconfirmed = async (err: any, index = 0) => {
    ...
    await inAppNotification(
      ...
      index === 0
        ...
        : `We couldn't confirm your comment on ${capitalize(
            post.integration?.providerIdentifier
          )}`,
      index === 0
        ...
        : `Your post is live on ${capitalize(
            post.integration?.providerIdentifier
          )}, but we couldn't confirm its comment was added. Please check your ${
            post?.integration?.name
          } account before adding it again to avoid duplicates.`,
      true,
      false,
      'fail'
    );
    ...
    for (let check = 0; check < maxPendingChecks; check++) {
      ...
      try {
        ...
        if (handle.type === 'bad-body') {
          ...
          await inAppNotification(
            post.organizationId,
            `Error posting${index === 0 ? ' ' : ' comments '}on ${
              post.integration?.providerIdentifier
  ...
  for (let i = 0; i < postsList.length; i++) {
    ...
    if (postsResults.length === before) {
      ...
      // outcome as unknown.
      try {
        await markUnconfirmed('Could not publish after several attempts', i);
      } catch (e) {
        /**empty**/
      }
      return false;
  ...
  while (list.length > 0) {
    ...
    if (todo.type === 'repeat-post') {
      await startChild(postWorkflowV113, {
