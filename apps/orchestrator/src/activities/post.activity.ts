...
import {
  NotificationService,
  NotificationType,
} from '@hookpost/nestjs-libraries/database/prisma/notifications/notification.service';
...
import {
  setHeartbeatDetails,
  withHeartbeat,
} from '@hookpost/nestjs-libraries/temporal/temporal.heartbeat';
import {
  BadBody,
  Disconnect,
...
export class PostActivity {
  ...
  private async postSocialBody(
    ...
    if (process.env.RAZORPAY_KEY_SECRET || process.env.STRIPE_SECRET_KEY) {
      const subscription = await this._subscriptionService.getSubscription(
        integration.organizationId
      );
