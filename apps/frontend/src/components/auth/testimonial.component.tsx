...
import {
  testimonials1,
  testimonials2,
} from '@hookpost/react/helpers/testomonials';
import { Testimonial } from '@hookpost/frontend/components/auth/testimonial';

export const TestimonialComponent = () => {
  // Nothing to show until there are real, consented quotes. Without this the
  // component still renders its marquee frame and gradients around an empty
  // column, which reads as a broken panel rather than an absent one.
  if (!testimonials1.length && !testimonials2.length) {
    return null;
  }
