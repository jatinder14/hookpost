  ...
  implements SocialProvider
{
  ...
  // Posting to a company Page genuinely needs the Community Management API
  // scopes, which require LinkedIn Partner Program approval - unlike the
  // personal provider, they cannot be trimmed away here without removing the
  // feature. Until that approval lands, connecting a Page fails at LinkedIn's
  // authorization screen ("Bummer, something went wrong"); the personal
  // LinkedIn channel is unaffected because it now requests only the self-serve
  // scopes. Override with LINKEDIN_PAGE_SCOPES once approved.
  // LinkedIn requires that Community Management API be the ONLY product on a
  // developer app ("for legal and security reasons"), so a Page app cannot also
  // carry Sign In with LinkedIn - which means `openid` and `profile` are not
  // available to it. Requesting a scope the app is not approved for makes
  // LinkedIn reject the entire authorization, so the default is exactly the
  // Community Management grant set and nothing more. Override with
  // LINKEDIN_PAGE_SCOPES if the app's product mix ever differs.
  // w_member_social is deliberately absent: every author/actor this provider
  // sends is a urn:li:organization, never a urn:li:person, so the Page app has
  // no reason to hold member write access - and asking for it would contradict
  // the Page-only use cases declared on the Community Management access form.
  override scopes = (
    process.env.LINKEDIN_PAGE_SCOPES ||
    'r_basicprofile rw_organization_admin r_organization_social w_organization_social'
  )
    .split(/[\s,]+/)
    .filter(Boolean);
  ...
  // The Page integration can run on a LinkedIn app of its own. Posting to a
  // company Page needs Community Management API approval, and LinkedIn does not
  // let a rejected request be re-submitted with the same app - so pointing Page
  // at a separate app keeps the personal LinkedIn app (approved and in use)
  // out of the blast radius, and means no connected channel has to be
  ...
  override async generateAuthUrl() {
    ...
    // No prompt=none — see linkedin.provider.ts: it breaks first-time connects.
  ...
  override async authenticate(params: {
    ...
    // /v2/userinfo is the OpenID Connect endpoint and needs `openid`, which a
    // Community-Management-only app does not have. Pick the identity source
    // from what was actually granted rather than from what we asked for, so
    // this works both on a CMA-only app and on an app that still carries Sign
    // In with LinkedIn.
    const granted = (scope || '').split(/[\s,]+/).filter(Boolean);
    const { id, name, picture, username } = granted.includes('openid')
      ? await this.identityFromUserinfo(accessToken)
      : await this.identityFromProfile(accessToken);
  ...
  private async identityFromUserinfo(accessToken: string) {
    const { name, sub, picture } = await (
      await fetch('https://api.linkedin.com/v2/userinfo', {
    ...
    return { id: sub, name, picture, username: vanityName };
  }

  private async identityFromProfile(accessToken: string) {
    const profile = await (
      await fetch(
