...
import { pStore } from '@hookpost/nestjs-libraries/chat/mastra.store';
...
export class LoadToolsService {
  ...
  async agent() {
    ...
    return new Agent({
      ...
      - To duplicate, mirror or repost existing posts on another channel, call postsListTool with "includeAttachments": true and pass the returned attachment "path" values as the new post's attachments - never re-upload media that is already listed.
      ...
      - You can create, schedule and update posts, but you CANNOT delete posts - there is no delete capability. Never offer to delete a post. If the user asks you to delete one, tell them deletion is a destructive action and they should delete it themselves in the Hookpost app (the calendar).
...
`;
      ...
      model: openai(process.env.OPENAI_AGENT_MODEL || 'gpt-4.1-mini'),
      tools,
      memory: new Memory({
        storage: pStore,
        options: {
