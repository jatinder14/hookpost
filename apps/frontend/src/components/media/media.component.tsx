...
import { VideoFrame } from '@hookpost/react/helpers/video.frame';
import { useUppyUploader } from '@hookpost/frontend/components/media/new.uploader';
import dynamic from 'next/dynamic';
import { useUser } from '@hookpost/frontend/components/layout/user.context';
import { AiImage } from '@hookpost/frontend/components/launches/ai.image';
import { DropFiles } from '@hookpost/frontend/components/layout/drop.files';
...
import { ThirdPartyMedia } from '@hookpost/frontend/components/third-parties/third-party.media';
import { ReactSortable } from 'react-sortablejs';
import { MediaComponentInner } from '@hookpost/frontend/components/launches/helpers/media.settings.component';
import { AiVideo } from '@hookpost/frontend/components/launches/ai.video';
import { useModals } from '@hookpost/frontend/components/layout/new-modal';
import { ThirdPartyMediaLibrary } from '@hookpost/frontend/components/third-parties/third-party.media-library';
...
const Polonto = dynamic(
  () => import('@hookpost/frontend/components/launches/polonto')
);
...
export const MediaBox: FC<{
  ...
  const addToUpload = useCallback(
    async (e: ChangeEvent<HTMLInputElement>) => {
      ...
      // so clear it immediately: without this, retrying the exact file that
      // just failed silently does nothing.
      e.target.value = '';
      ...
      try {
        ...
        // addFiles throws synchronously on a rejected file, which would leave
        // the button spinning and disabled with no upload ever starting.
        setLoading(false);
      }
    },
    [toaster, t, uppy]
...
export const MultiMediaComponent: FC<{
  ...
  // Polotno is a paid commercial SDK and the licence is a fixed ~$249/mo
  // regardless of usage - 37 paying users of gross margin against a ~5 user
  // break-even. Until it is licensed, the editor stays hidden rather than
  // running unlicensed in demo mode. Set NEXT_PUBLIC_POLOTNO and rebuild the
  // frontend (NEXT_PUBLIC_* is inlined at build time) to bring it back; no
  // code change needed.
  const { plontoKey } = useVariables();
  const designEditorEnabled = !!plontoKey;

  const designMedia = useCallback(() => {
    if (!!user?.tier?.ai && !dummy && designEditorEnabled) {
      modals.openModal({
        askClose: false,
  ...
  }, [changeMedia, t, designEditorEnabled]);
...
export const MediaComponent: FC<{
  ...
  // See the note on designEditorEnabled above - same licence gate.
  const { plontoKey: innerPlontoKey } = useVariables();
  const innerDesignEditorEnabled = !!innerPlontoKey;

  const showDesignModal = useCallback(() => {
    if (!innerDesignEditorEnabled) {
      return;
    }
    modals.openModal({
