...
export const setSentryUserContext = (params: {
  ...
  try {
    ...
    if (params.paymentId) {
      Sentry.setTag('payment.customer_id', params.paymentId);
      if (params.paymentId.startsWith('sub_') || params.paymentId.startsWith('cust_')) {
        Sentry.setTag('razorpay.customer_id', params.paymentId);
      }
    }
  } catch (err) {
  ...
  try {
    Sentry.init({
      initialScope: {
        ...
        contexts: {
          app: {
            name: `Hookpost ${capitalize(appName)}`,
          },
        },
      },
