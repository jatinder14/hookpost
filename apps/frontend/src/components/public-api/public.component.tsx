...
import { useDecisionModal } from '@hookpost/frontend/components/layout/new-modal';
...
const getMcpConfig = (
  ...
  if (method === 'path') {
    switch (client) {
      case 'Claude Code':
        return {
          config: `claude mcp add hookpost --transport http "${urlWithKey}"`,
          hint: 'Run this command in your terminal.',
        };
      case 'Cursor':
        return {
          config: json({ mcpServers: { hookpost: { url: urlWithKey } } }),
          hint: 'Add to .cursor/mcp.json in your project root.',
        };
      case 'VS Code / Copilot':
        return {
          config: json({
            servers: { hookpost: { type: 'http', url: urlWithKey } },
          }),
      ...
      case 'Windsurf':
        return {
          config: json({
            mcpServers: { hookpost: { serverUrl: urlWithKey } },
          }),
          hint: 'Add to ~/.codeium/windsurf/mcp_config.json',
        };
      case 'Amp':
        return {
          config: `amp mcp add hookpost ${urlWithKey}`,
          hint: 'Run this command in your terminal.',
        };
      case 'Codex':
        return {
          config: `# ~/.codex/config.toml\n\n[mcp_servers.hookpost]\nurl = "${urlWithKey}"`,
      ...
      case 'Warp':
        return {
          config: json({ hookpost: { url: urlWithKey } }),
  ...
  switch (client) {
    case 'Claude Code':
      return {
        config: `claude mcp add --transport http hookpost ${urlBase} --header "Authorization: ${bearer}"`,
        hint: 'Run this command in your terminal.',
      };
    case 'Cursor':
      return {
        config: json({
          mcpServers: {
            hookpost: { url: urlBase, headers: { Authorization: bearer } },
          },
        }),
        hint: 'Add to .cursor/mcp.json in your project root.',
      };
      ...
      <div className="bg-newBgColorInner px-[20px] py-[14px] border-b border-newBorder flex items-start justify-between gap-[12px]">
        <div>
          ...
          <div className="text-[13px] text-customColor18 mt-[2px]">
            {t(
              'connect_your_mcp_client_to_hookpost_to_schedule_your_posts_faster',
              'Connect Hookpost MCP server to your client (Http streaming) to schedule your posts faster.'
            )}
          </div>
        </div>
        <div className="flex gap-[6px] shrink-0 pt-[2px]">
          {billingEnabled && (
            <a
              ...
              href="https://hookpost.hookstep.in/mcp"
              target="_blank"
            >
          ...
          <a
            className="cursor-pointer px-[16px] h-[36px] bg-[#612BD3] hover:bg-[#5520CB] text-white transition-colors rounded-[8px] text-[13px] font-[600] flex items-center gap-[6px]"
            href="https://hookpost.hookstep.in/guides/claude-mcp-social-media"
            target="_blank"
          >
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6" /><polyline points="15 3 21 3 21 9" /><line x1="10" y1="14" x2="21" y2="3" /></svg>
...
// Only list a package here once it is actually live on npm - the previous
// version of this section advertised installs that all 404'd. Verified with
// `npm view <name> version` before being added.
const tools = [
  {
    name: 'Node.js SDK',
    npm: '@hookpost/node',
    install: 'npm install @hookpost/node',
    description:
      'Call the API from your own code, with types. Works in ESM and CommonJS.',
    href: 'https://www.npmjs.com/package/@hookpost/node',
  },
  {
    name: 'Command line',
    npm: 'hookpost',
    install: 'npm install -g hookpost',
    description:
      'Schedule, list and delete posts from your terminal or a CI job.',
    href: 'https://www.npmjs.com/package/hookpost',
  },
  {
    name: 'n8n node',
    npm: 'n8n-nodes-hookpost',
    install: 'n8n-nodes-hookpost',
    description:
      'Post from an n8n workflow. Settings > Community Nodes > Install.',
  ...
  return (
    <div className="bg-newBgColorInnerInner rounded-[12px] border border-newBorder overflow-hidden">
      <div className="bg-newBgColorInner px-[20px] py-[14px] border-b border-newBorder">
        <div className="text-[15px] font-[600]">{t('tools', 'Tools')}</div>
        <div className="text-[13px] text-customColor18 mt-[2px]">
          {t(
            'tools_description',
            'Official packages that talk to the API above. They use the same API Key.'
          )}
        </div>
      </div>
      <div className="p-[20px] flex flex-col gap-[16px]">
        {tools.map((tool) => (
          <div
            key={tool.npm}
            className="flex flex-col gap-[8px] pb-[16px] last:pb-0 border-b last:border-b-0 border-newBorder"
          >
            <div className="flex items-center justify-between gap-[12px]">
              <div>
                <div className="text-[14px] font-[600]">{tool.name}</div>
                <div className="text-[13px] text-customColor18 mt-[2px]">
                  {tool.description}
                </div>
              </div>
              <a
                className="cursor-pointer px-[16px] h-[36px] bg-btnSimple hover:bg-boxHover transition-colors rounded-[8px] text-[13px] font-[600] flex items-center gap-[6px] shrink-0"
                href={tool.href}
                target="_blank"
                rel="noreferrer"
              >
            ...
            <div className="flex items-center gap-[8px]">
              <pre className="flex-1 bg-newBgColorInner border border-newBorder rounded-[8px] px-[16px] h-[44px] flex items-center text-[13px] overflow-x-auto">
                {tool.install}
              </pre>
              <CopyButton text={tool.install} label={t('copy', 'Copy')} />
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="flex flex-col gap-[40px]">
      <div className="text-[14px] text-textColor leading-[1.7]">
        ...
        {t(
          'api_auth_note_line2',
          'If you are building a product that schedules posts on behalf of other Hookpost users,'
        )}
        <br />
        {t(
          'api_auth_note_line3',
      ...
      <div className="bg-newBgColorInnerInner rounded-[12px] border border-newBorder overflow-hidden">
        <div className="bg-newBgColorInner px-[20px] py-[14px] border-b border-newBorder flex items-start justify-between gap-[12px]">
          <div>
            ...
            <div className="text-[13px] text-customColor18 mt-[2px]">
              {t(
                'use_hookpost_api_to_integrate_with_your_tools',
                'Use Hookpost API to integrate with your tools.'
              )}
            </div>
          </div>
          <div className="flex gap-[6px] shrink-0 pt-[2px]">
            <a
              className="cursor-pointer px-[16px] h-[36px] bg-[#612BD3] hover:bg-[#5520CB] text-white transition-colors rounded-[8px] text-[13px] font-[600] flex items-center gap-[6px]"
              href="https://hookpost.hookstep.in/docs/public-api"
              target="_blank"
            >
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6" /><polyline points="15 3 21 3 21 9" /><line x1="10" y1="14" x2="21" y2="3" /></svg>
              {t('read_the_docs', 'Docs')}
