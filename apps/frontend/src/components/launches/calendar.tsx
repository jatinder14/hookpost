...
import {
  ...
  Integrations,
  useCalendar,
} from '@hookpost/frontend/components/launches/calendar.context';
...
import { useAddProvider } from '@hookpost/frontend/components/launches/add.provider.component';
...
import { StatisticsModal } from '@hookpost/frontend/components/launches/statistics';
import { MissingReleaseModal } from '@hookpost/frontend/components/launches/missing-release.modal';
import { useT } from '@hookpost/react/translation/get.transation.service.client';
import i18next from 'i18next';
import { AddEditModal } from '@hookpost/frontend/components/new-launch/add.edit.modal';
import { CreationMethodBadge } from '@hookpost/frontend/components/launches/creation.method.badge';
  ...
  return (
    <div className="flex flex-col text-textColor flex-1">
      <div className="flex-1 relative">
        <div
          className={clsx(
            'grid gap-[4px] rounded-[10px] absolute h-full start-0 top-0 w-full overflow-auto scrollbar scrollbar-thumb-fifth scrollbar-track-newBgColor',
            // A narrow screen cannot show seven readable day columns at once.
            // minmax(0,1fr) let them collapse to ~23px, which stacked the day
            // names and dates on top of each other. Give each column a floor
            // and let the container scroll sideways instead.
            // The floor stays at every width. At 1440px each column resolves
            // to ~123px so it behaves exactly like minmax(0,1fr); it only
            // bites when there genuinely is not room, which is when we want
            // horizontal scrolling. Switching to minmax(0,1fr) at md collapsed
            // tablet columns to 27px - the same overlap, moved to 768px.
            '[grid-template-columns:52px_repeat(7,minmax(88px,1fr))]',
            'md:[grid-template-columns:136px_repeat(7,minmax(88px,1fr))]'
          )}
        >
          ...
          {localizedDays.map((day, index) => (
            ...
            >
              <div className="text-[12px] md:text-[14px] font-[500] text-newTableText truncate max-w-full">
                {day.name}
              </div>
              <div
                className={clsx(
                  'text-[12px] md:text-[14px] font-[600] flex items-center justify-center gap-[4px] md:gap-[6px] whitespace-nowrap',
          ...
          {hours.map((hour) => (
            <Fragment key={hour}>
              <div className="p-1 md:p-2 md:pe-4 text-center items-center justify-center flex text-[11px] md:text-[14px] text-newTableText whitespace-nowrap">
  ...
  return (
    <div className="flex flex-col text-textColor flex-1">
      <div className="flex-1 flex relative">
        <div
          className={clsx(
            'grid grid-rows-[62px_auto] gap-[4px] rounded-[10px] absolute start-0 top-0 overflow-auto w-full h-full scrollbar scrollbar-thumb-tableBorder scrollbar-track-secondary',
        ...
        >
          {localizedDays.map((day) => (
            <div
              key={day}
              className="z-[20] p-1 md:p-2 bg-newTableHeader flex justify-center items-center flex-col h-[62px] rounded-[8px] sticky top-0 overflow-hidden"
            >
              {/* Full weekday names ("Wednesday") do not fit a 78px mobile
                  column, so the cell overflowed. Truncate rather than widen -
                  a month grid needs all seven columns on screen at once. */}
              <div className="text-[12px] md:text-[14px] truncate max-w-full">
                {day}
              </div>
            </div>
          ))}
