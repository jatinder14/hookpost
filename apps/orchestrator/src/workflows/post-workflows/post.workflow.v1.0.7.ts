...
// heartbeatTimeout instead of the full startToCloseTimeout. 3 minutes keeps
// the SDK's 60s throttled heartbeat sends far from the limit - a false
// timeout here retries and could duplicate a comment, so it must only fire
// for a genuinely dead worker. Only heartbeating activities may live behind
// this proxy - a non-heartbeating one would be killed after 3 minutes.
const proxyCommentTaskQueue = (taskQueue: string) => {
  return proxyActivities<PostActivity>({
    startToCloseTimeout: '30 minute',
    heartbeatTimeout: '3 minute',
    taskQueue,
    retry: {
      maximumAttempts: 3,
      backoffCoefficient: 1,
...
// bounds how long a crashed worker leaves the post in limbo. 3 minutes keeps
// the SDK's 60s throttled heartbeat sends far from the limit, so it only
// fires for a genuinely dead worker, never a slow-but-alive publish.
