  ...
  override handleErrors(body: string):
    ...
    if (body.includes('The media could not be fetched from this URI')) {
      return {
        type: 'bad-body',
        value:
          "One of the media URLs is invalid or inaccessible, make sure it's being uploaded to Hookpost first",
      };
    }
  ...
  async generateAuthUrl() {
    ...
    return {
      url:
        ...
        // Required by Threads' /oauth/authorize - unlike Facebook and Instagram,
        // which use /dialog/oauth and default to a code response. Omitting it
        // makes Threads fail after the user logs in, with the thoroughly
        // misleading "Authorisation failed: No app ID was sent with the
        // request." (error_code 4476002) even though client_id is present.
        `&response_type=code`,
