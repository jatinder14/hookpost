...
import { setHeartbeatDetails } from '@hookpost/nestjs-libraries/temporal/temporal.heartbeat';
...
export class YoutubeProvider extends SocialAbstract implements SocialProvider {
  ...
  // Kept to the minimum Google will approve: every sensitive scope here maps to
  // a call this provider actually makes, because OAuth verification requires
  // each one to be justified on camera.
  //   youtube.upload        -> the resumable video upload and thumbnails.set
  //   youtube.readonly      -> channels.list (channel name/avatar/stats), videos.list
  //   yt-analytics.readonly -> youtubeAnalytics.reports.query on the analytics page
  // Deliberately NOT requested: 'youtube' and 'youtubepartner' (no videos.update
  // or partner/Content-ID call exists here - thumbnails.set is covered by
  // youtube.upload) and 'youtube.force-ssl' (no captions, comments or live).
  // Narrowing this list is safe for already-connected channels: their tokens
  // carry more scopes than required, so checkScopes still passes.
  scopes = [
