...
} from '@hookpost/nestjs-libraries/dtos/posts/providers-settings/facebook.dto';
 ...
 * edge only lists Pages the person administers directly - and enumerating them
 * the documented way (/me/businesses -> owned_pages) needs business_management,
 * which we deliberately do not request (see the note on the provider below).
 * debug_token already reports pages_show_list's target_ids, and reading those
 * ids one by one works with a plain user token, so this recovers Business-owned
 * Pages without asking for the extra permission.
 */
export const grantedPageIds = async (userToken: string): Promise<string[]> => {
  ...
  try {
    ...
    // A token we cannot introspect just means no extra Pages to add.
    return [];
  }
};
...
export class FacebookProvider extends SocialAbstract implements SocialProvider {
  ...
  // of the consent screen. Verified on v25.0 against app 1011413801885356:
  // requesting it alone fails, requesting it together with
  // pages_read_user_content fails identically, and dropping it gives a clean
  // consent screen. Since the permission can never actually be granted,
  // asking for it buys nothing and breaks the flow for anyone holding a role
  // on the app. Cost: comment() cannot post comments on Facebook Page posts -
  // it would have failed at runtime anyway without the grant.
  scopes = [
    'pages_show_list',
    'pages_manage_posts',
  ...
  async pages(accessToken: string) {
    ...
    // /me/accounts and the Business Manager edges both miss Pages owned through
    // a Business portfolio, so fall back to reading the granted ids directly.
    for (const pageId of await grantedPageIds(accessToken)) {
  ...
  async fetchPageInformation(accessToken: string, data: { page: string }) {
    ...
    // 3. Business-portfolio-owned Pages are absent from both edges above; read
    //    the id straight off the token's grant.
    if ((await grantedPageIds(accessToken)).includes(String(pageId))) {
      const page = await (
        await fetch(
