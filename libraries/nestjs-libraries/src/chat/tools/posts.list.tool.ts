import { AgentToolInterface } from '@hookpost/nestjs-libraries/chat/agent.tool.interface';
import { createTool } from '@mastra/core/tools';
import { z } from 'zod';
import { Injectable } from '@nestjs/common';
import { PostsService } from '@hookpost/nestjs-libraries/database/prisma/posts/posts.service';
import { checkAuth } from '@hookpost/nestjs-libraries/chat/auth.context';
...
Pass "includeAttachments": true to also get each post's "attachments" (media already uploaded to Hookpost). Their "path" values can be passed straight to integrationSchedulePostTool as "attachments" to reuse the same media on another channel - do not re-upload them.
Posts cannot be deleted through the Hookpost tools - if the user wants to delete a post, tell them to do it themselves in the Hookpost app; never offer to delete a post.
`,
